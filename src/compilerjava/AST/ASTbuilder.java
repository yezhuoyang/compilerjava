...
public class ASTbuilder extends MXBaseVisitor<Node>{
	...
	public Node visitFunctionDecl(MXParser.FunctionDeclContext ctx) {
	        tpNode tpnode=ctx.typeForfunc()==null? null:(tpNode) visit(ctx.typeForfunc());
	...
	@Override
	 public Node visitVariableDecl(MXParser.VariableDeclContext ctx) {
          tpNode tpnode=(tpNode) visit(ctx.type());
	...
	public Node visitArrayType(MXParser.ArrayTypeContext ctx) {
         return new arraytpNode((tpNode)visit(ctx.type()),new position(ctx.getStart()));
	}
	...
	public Node visitArrayCreator(MXParser.ArrayCreatorContext ctx) {
	       ...
	       return new newexprNode( (tpNode)visit(ctx.nonArraytype()),
